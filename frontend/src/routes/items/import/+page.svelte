<script lang="ts">
	...
	async function analyzeFile() {
		...
		try {
			const res = await fetch('/api/items/import/analyze', { method: 'POST', body: formData });
			if (!res.ok) {
	...
	async function executeImport() {
		...
		try {
			const res = await fetch('/api/items/import/execute', {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ fileToken: analysis.fileToken, columnMap }),
			});
			if (!res.ok) {
				const text = await res.text();
				throw new Error(text);
...
{:else if result}
	<div class="card">
		...
		<a href="/items" class="btn btn-primary" style="margin-top:16px">Back to Items</a>
	</div>
	...
	{#if analysis}
		<div class="card" style="margin-top:10px">
			...
			<div class="import-actions">
				...
				<button class="btn btn-primary" onclick={executeImport} disabled={importing || fields.some(f => f.required && !csvColFor(f.value))}>
					{importing ? 'Importing...' : `Import ${analysis.totalRows} Rows`}
				</button>
			</div>
		</div>
	{/if}
