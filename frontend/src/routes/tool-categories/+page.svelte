<script lang="ts">
	...
	function load() { loading = true; toolCategories.list().then(l => list = l).finally(() => loading = false); }
	$effect(load);
	...
	function add() {
		if (!name.trim()) return;
		toolCategories.create({ name: name.trim() }).then(() => { name = ''; load(); addToast('Category added', 'success'); }).catch(e => addToast(e.message, 'error'));
	}

	function remove(id: number) {
		toolCategories.remove(id).then(() => { load(); addToast('Category removed', 'success'); }).catch(e => addToast(e.message, 'error'));
	}
</script>
	...
	{:else}
		<div class="table-wrap">
			<table>
				...
				<tbody>
					{#each list as v}
						<tr data-id={v.id} class:highlight={v.id === highlightId} onclick={() => goto(`/tool-categories/${v.id}`)}>
							<td>{v.id}</td>
							<td><a href="/tool-categories/{v.id}" onclick={(e) => e.stopPropagation()}>{v.name}</a></td>
